...
const readDB = (f) => {
    if (!fs.existsSync(f)) {
        ...
        if (f === DB.products) return [{ name: 'Su', stok: 100, minStok: 10 }, { name: 'Kola', stok: 50, minStok: 10 }];
...
app.post('/api/logs', (req, res) => {
    ...
    if (req.body.items && Array.isArray(req.body.items)) {
        ...
        req.body.items.forEach(item => {
            ...
            if (p) p.stok = Math.max(0, (p.stok || 0) - item.count);
        });
...
app.post('/api/stok-islem', (req, res) => {
    ...
    if (p) {
        ...
        if (isNaN(islemMiktari) || islemMiktari <= 0) return res.json({ success: false, message: 'Gecersiz miktar' });
        if (type === 'giris') {
            p.stok = (p.stok || 0) + islemMiktari;
        } else {
            if ((p.stok || 0) < islemMiktari) return res.json({ success: false, message: 'Yetersiz stok' });
            p.stok = (p.stok || 0) - islemMiktari;
        }
        stokLogs.unshift({
            productName,
        ...
        res.json({ success: false, message: 'Urun bulunamadi' });
    }
});
...
app.delete('/api/products/:name', (req, res) => {
    ...
    products = products.filter(x => x.name !== decodeURIComponent(req.params.name));
    if (products.length === before) return res.json({ success: false, message: 'Urun bulunamadi' });
    writeDB(DB.products, products);
    res.json({ success: true });
});

// YENİ: Minimum stok eşiği güncelleme
app.patch('/api/products/:name/minstok', (req, res) => {
    let products = readDB(DB.products);
    const p = products.find(x => x.name === decodeURIComponent(req.params.name));
    if (!p) return res.json({ success: false, message: 'Urun bulunamadi' });
    const minStok = parseInt(req.body.minStok);
    if (isNaN(minStok) || minStok < 0) return res.json({ success: false, message: 'Gecersiz deger' });
    p.minStok = minStok;
    writeDB(DB.products, products);
    res.json({ success: true });
});
...
<head>
    ...
    <style>
        ...
        /* Kritik stok banner */
        #kritikBanner { display:none; position:fixed; top:0; left:0; right:0; background:var(--r); color:white; text-align:center; padding:10px 40px 10px 10px; font-weight:bold; font-size:13px; z-index:99999; animation:bannerPulse 2s infinite; }
        #kritikBanner .banner-close { position:absolute; right:12px; top:50%; transform:translateY(-50%); font-size:18px; cursor:pointer; }
        @keyframes bannerPulse { 0%,100%{background:#e74c3c} 50%{background:#c0392b} }
        .btn-sm { padding:5px 10px; font-size:11px; border-radius:5px; cursor:pointer; border:none; font-weight:bold; margin:0 2px; }
        .btn-del { background:#fadbd8; color:var(--r); }
        .btn-edit { background:#d6eaf8; color:var(--b); }
        .minstok-input { width:60px; padding:4px 6px; margin:0; border:1px solid #ddd; border-radius:5px; font-size:12px; display:inline-block; }
    </style>
</head>
<body>

    <div id="kritikBanner">
        <span id="kritikBannerText"></span>
        <span class="banner-close" onclick="document.getElementById('kritikBanner').style.display='none'">✕</span>
    </div>
    ...
    <div id="adminPage" class="page">
        ...
        <div id="t_stok" class="tab-content">
            ...
            <table>
                <thead><tr><th>Ürün</th><th>Mevcut Stok</th><th>Min. Eşik</th><th>İşlem</th></tr></thead>
                <tbody id="stokListBody"></tbody>
            </table>
        ...
        <div id="t_setup" class="tab-content">
            ...
            <h4>Ürün</h4>
            <div id="pList"></div>
            <input id="inP" placeholder="Ürün Adı">
            <input id="inPS" type="number" placeholder="Başlangıç Stoğu">
            <input id="inPMin" type="number" placeholder="Minimum Stok Eşiği (varsayılan: 10)">
            <button class="btn-p" onclick="addProd()">Ekle</button>
        </div>
        <div id="t_end" class="tab-content" style="text-align:center">
            <button class="btn-p" onclick="window.open('/api/export')" style="background:var(--g)">EXCEL İNDİR</button>
            <button class="btn-p" onclick="endDay()" style="background:var(--r)">SIFIRLA</button>
        </div>
    </div>
    ...
    <script>
        ...
        async function login() {
            const res = await fetch('/api/login', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({user:document.getElementById('lUser').value, pass:document.getElementById('lPass').value}) });
        ...
        function launchApp() {
            ...
            else { document.getElementById('staffPage').classList.add('active'); document.getElementById('sn').innerText = "P: " + currentUser.user; initStaff(); }
        }

        // YENİ: Kritik stok banner — eşiğin altındaki ürünleri ekranda gösterir
        function checkKritikStok() {
            const kritikler = products.filter(p => (p.stok || 0) <= (p.minStok !== undefined ? p.minStok : 10));
            const banner = document.getElementById('kritikBanner');
            const bannerText = document.getElementById('kritikBannerText');
            if (kritikler.length > 0) {
                bannerText.innerText = '⚠️ KRİTİK STOK: ' + kritikler.map(p => p.name + ' (' + (p.stok||0) + ' adet)').join(' | ');
        ...
        async function autoUpdate() {
            ...
            const view_rooms = document.getElementById('view_rooms');
            const adminPage = document.getElementById('adminPage');
            const t_live = document.getElementById('t_live');
            const t_matrix = document.getElementById('t_matrix');
            const t_stok = document.getElementById('t_stok');
        ...
        async function initStaff() {
            ...
            hotelData = s; products = p; logs = l; notes = n;
            checkKritikStok();
            renderBlocks();
        }
        ...
        function renderBlocks() {
            ...
            const view_floors = document.getElementById('view_floors');
            const view_rooms = document.getElementById('view_rooms');
        ...
        function selectBlock(n) {
            ...
            const view_floors = document.getElementById('view_floors');
            const view_rooms = document.getElementById('view_rooms');
            blockTabs.style.display='none'; view_floors.style.display='grid'; view_rooms.style.display='none';
        ...
        function selectFloor(n) {
            selFloor = n;
            const b = hotelData.find(x => x.name === selBlock);
            if (!b) return;
            const f = b.floors.find(x => x.name === n);
            if (!f) return;
            const view_floors = document.getElementById('view_floors');
            const view_rooms = document.getElementById('view_rooms');
        ...
        function openStatusMenu(r) {
            ...
            const targetRoomTitle = document.getElementById('targetRoomTitle');
            const staffNoteView = document.getElementById('staffNoteView');
            staffContent.style.display='none'; statusScreen.style.display='block'; productMenu.style.display='none';
        ...
        function openProductMenu() {
            ...
            pGrid.innerHTML = products.map((p, i) => {
                const minStok = p.minStok !== undefined ? p.minStok : 10;
                const kritik = (p.stok || 0) <= minStok;
                return '<div style="border:' + (kritik ? '2px solid var(--r)' : '1px solid #ddd') + ';padding:5px;text-align:center;border-radius:8px;background:' + (kritik ? '#fff5f5' : '#fff') + '" onclick="counts[\\'' + p.name + '\\']++; document.getElementById(\\'c'+i+'\\').innerText=counts[\\'' + p.name + '\\']">'
                    + p.name + '<br><small style="color:' + (kritik ? 'var(--r)' : '#666') + '">Stok: ' + (p.stok||0) + (kritik ? ' ⚠️' : '') + '</small><br><b id="c'+i+'" style="color:var(--b);font-size:20px">0</b></div>';
            }).join('');
        }
        ...
        // YENİ: Ayarlar sekmesinde ürün listesi (silme butonu ile)
        function renderProdSetupList() {
            document.getElementById('pList').innerHTML = products.length === 0
                ? '<p style="color:#aaa;font-size:12px">Henüz ürün yok.</p>'
                : '<div style="margin-bottom:10px">' + products.map(x =>
                    '<div style="display:flex;align-items:center;justify-content:space-between;padding:6px 0;border-bottom:1px solid #eee">'
                    + '<span style="font-size:13px"><b>' + x.name + '</b> <span style="color:#aaa">(Stok: ' + (x.stok||0) + ')</span></span>'
                    + '<button class="btn-sm btn-del" onclick="delProd(\\'' + x.name + '\\')">🗑 Sil</button>'
                    + '</div>'
                ).join('') + '</div>';
        }

        async function refreshStokTab() {
            const stokProdSelect = document.getElementById('stokProdSelect');
            const stokListBody = document.getElementById('stokListBody');
            const stokHistoryBody = document.getElementById('stokHistoryBody');
            stokProdSelect.innerHTML = products.map(p => '<option value="' + p.name + '">' + p.name + '</option>').join('');
            // YENİ: Min. Eşik sütunu + kaydet butonu + sil butonu
            stokListBody.innerHTML = products.map(p => {
                const minStok = p.minStok !== undefined ? p.minStok : 10;
                const kritik = (p.stok || 0) <= minStok;
                const safeId = p.name.replace(/[^a-zA-Z0-9]/g, '_');
                return '<tr>'
                    + '<td>' + p.name + '</td>'
                    + '<td class="' + (kritik ? 'stok-kritik' : '') + '">' + (p.stok||0) + (kritik ? ' ⚠️' : '') + '</td>'
                    + '<td><input type="number" class="minstok-input" id="ms_' + safeId + '" value="' + minStok + '" min="0"> <button class="btn-sm btn-edit" onclick="saveMinStok(\\'' + p.name + '\\')">Kaydet</button></td>'
                    + '<td><button class="btn-sm btn-del" onclick="delProd(\\'' + p.name + '\\')">🗑 Sil</button></td>'
                    + '</tr>';
            }).join('');
        ...
        async function submitStokIslem(type) {
            const stokProdSelect = document.getElementById('stokProdSelect');
            const stokAmount = document.getElementById('stokAmount');
            const stokSource = document.getElementById('stokSource');
            ...
            const res = await fetch('/api/stok-islem', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(body) });
            const data = await res.json();
            if(!data.success) return alert(data.message || "İşlem başarısız!");
            stokAmount.value = "";
            autoUpdate();
            alert("İşlem başarıyla kaydedildi.");
        }

        // YENİ: Minimum stok eşiği kaydetme
        async function saveMinStok(productName) {
            const safeId = productName.replace(/[^a-zA-Z0-9]/g, '_');
            const el = document.getElementById('ms_' + safeId);
            if (!el) return;
            const val = parseInt(el.value);
            if (isNaN(val) || val < 0) return alert("Geçerli bir değer giriniz!");
            const res = await fetch('/api/products/' + encodeURIComponent(productName) + '/minstok', {
                method: 'PATCH',
                headers: {'Content-Type':'application/json'},
                body: JSON.stringify({ minStok: val })
            });
            const data = await res.json();
            if (!data.success) return alert(data.message || "Hata!");
            const p = products.find(x => x.name === productName);
            if (p) p.minStok = val;
            checkKritikStok();
            alert(productName + " için minimum eşik " + val + " olarak kaydedildi.");
        }
        ...
        async function delProd(name) {
            ...
            const data = await res.json();
            if (!data.success) return alert(data.message || "Silinemedi!");
        ...
        function refreshMatrix() {
            ...
            hotelData.forEach(b => {
                ...
                b.floors.forEach(f => {
                    h += '<div class="grid">';
                    f.rooms.forEach(r => {
                        const lastLog = logs.find(l => String(l.room) === String(r));
                        const roomLogs = logs.filter(l => String(l.room) === String(r));
                        const histLogs = [...roomLogs].reverse();
                        const noteCls = notes[r] ? 'note-bg has-note' : '';
                        let hist = '<div class="history-container">' + histLogs.map(rl => '<div class="h-bar h-'+rl.status+'"></div>').join('') + '</div>';
        ...
        function showRoomDetail(r) {
            ...
            document.getElementById('mBody').innerHTML = (notes[r] ? '<div class="staff-note-box">GÜNCEL NOT: '+notes[r]+'</div>' : '') + roomLogs.map(l => '<div><b>'+l.endTime+'</b>: '+l.status+' ('+l.staff+')<br><small>'+l.details+'</small></div>').join('<hr>');
        ...
        async function addStaff() {
            ...
            const inUP = document.getElementById('inUP');
            await fetch('/api/users', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({user:inUN.value, pass:inUP.value}) });
            initAdmin(); inUN.value=''; inUP.value='';
        }
        ...
        async function addProd() {
            ...
            if (!inP.value.trim()) return alert("Ürün adı giriniz!");
            let p = products;
            p.push({ name: inP.value.trim(), stok: parseInt(inPS.value)||0, minStok: parseInt(inPMin.value)||10 });
            await fetch('/api/products', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(p) });
            inP.value=''; inPS.value=''; inPMin.value='';
...
app.listen(PORT, '0.0.0.0', () => console.log(`v17.5 Stok Yonetimi Aktif: http://localhost:${PORT}`));
