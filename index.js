...
app.post('/api/logs', (req, res) => {
    ...
    if (req.body.items && Array.isArray(req.body.items)) {
        ...
        req.body.items.forEach(item => {
            ...
            // DÜZELTME: Stok negatife düşmesin diye kontrol eklendi
...
app.post('/api/stok-islem', (req, res) => {
    ...
    if (p) {
        ...
        if (isNaN(islemMiktari) || islemMiktari <= 0) {
        ...
        if (type === 'giris') {
            ...
            // DÜZELTME: Stok negatife düşmesin
            if ((p.stok || 0) < islemMiktari) {
    ...
    <script>
        ...
        async function autoUpdate() {
            ...
            if(adminPage.classList.contains('active')) {
                ...
                // DÜZELTME: Stok tabı sadece aktifse fetch yapıyor (performans)
        ...
        function selectFloor(n) {
            ...
            view_rooms.innerHTML = f.rooms.map(r => {
                // DÜZELTME: logs dizisinin başı en yeni kayıt (unshift ile ekleniyor), ilk eşleşen doğru
        ...
        // DÜZELTME: Object.entries destructuring düzeltildi — artık ürünler doğru kaydediliyor
        ...
        async function refreshStokTab() {
            ...
            stokListBody.innerHTML = products.map(p => \`<tr><td>\${p.name}</td><td class="\${(p.stok||0) < 10 ? 'stok-kritik' : ''}">\${p.stok || 0}</td></tr>\`).join('');
        ...
        // DÜZELTME: refreshMatrix'te .reverse() kopya dizi üzerinde yapılıyor, orijinal logs bozulmuyor
        // lastLog: logs başında en yeni kayıt olduğundan doğrudan logs.find() ile alınıyor
        function refreshMatrix() {
            ...
            hotelData.forEach(b => {
                ...
                b.floors.forEach(f => {
                    h += '<div class="grid">';
                    f.rooms.forEach(r => {
                        // DÜZELTME: logs zaten unshift ile ekleniyor, [0] en yeni kayıt
                        const lastLog = logs.find(l => String(l.room) === String(r));
                        // Tarihçe için kopya dizide reverse (orijinal logs bozulmuyor)
        ...
        async function addProd() {
            ...
            initAdmin(); inP.value=''; inPS.value='';
        }
